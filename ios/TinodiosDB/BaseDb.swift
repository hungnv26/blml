...
public class BaseDb {
    ...
    // All shared-container consumers must fall back to app-local storage then.
    // The only real loss is that the notification extension can't reach the DB.
    public static let kAppGroupAvailable = FileManager.default
        .containerURL(forSecurityApplicationGroupIdentifier: "group." + BaseDb.kBundleId) != nil
    ...
    private init() {
        ...
        var documentsDirectory = containerUrl.absoluteString
