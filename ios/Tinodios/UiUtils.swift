...
class UiUtils {
    ...
    // Maximum size of the avatar image in pixels. 800px keeps the large
    // profile circle sharp on 3x Retina displays; 384 was visibly soft.
    static let kMaxAvatarSize: CGFloat = 800
    // Maximum linear size of an image.
    static let kMaxBitmapSize: CGFloat = 1024
    // Maximum size of avatar preview when it's sent out-of-band. 96px so the
    // placeholder shown before the full image loads is not a blur.
    static let kAvatarPreviewDimensions: CGFloat = 96
    ...
    // WhatsApp-style root: a tab bar with Chats and Settings. The chats
    // UINavigationController lives in the first tab; everything that used to
    // assume the nav controller was the window root resolves it via mainNavVC().
    public static func makeMainTabVC() -> UITabBarController {
        let storyboard = UIStoryboard(name: "Main", bundle: nil)

        let chatsNav = storyboard.instantiateViewController(withIdentifier: "ChatsNavigator") as! UINavigationController
        chatsNav.tabBarItem = UITabBarItem(
            title: NSLocalizedString("Chats", comment: "Tab title"),
        ...
        // NOTE: no Calls tab — the app has an active-call UI (CallViewController)
        // but no call-history screen to list past calls. Adding one means building
        // a new view over the webrtc call events stored in-band with messages.
        let tabs = UITabBarController()
        tabs.viewControllers = [chatsNav, contactsNav, settingsNav]
        tabs.tabBar.tintColor = UIColor(fromHexCode: 0xff00a884)
        return tabs
    }

    // The chats navigation controller, whether the window root is the tab bar
    // (normal) or a bare navigation controller (legacy paths).
    public static func mainNavVC() -> UINavigationController? {
        ...
        if let tabs = root as? UITabBarController {
            return tabs.viewControllers?.first as? UINavigationController
        }
        return nil
    }
    ...
    public static func routeToChatListVC() {
        DispatchQueue.main.async {
            if let window = (UIApplication.shared.delegate as! AppDelegate).window {
                window.rootViewController = UiUtils.makeMainTabVC()
            }
    ...
    private static func presentedMessageVC(forTopic topicName: String) -> MessageViewController? {
        guard let rootVC = UiUtils.mainNavVC() else {
            return nil
        }
    ...
    public static func routeToMessageVC(forTopic topicId: String, completion: ((MessageViewController?) -> (Void))? = nil) {
        DispatchQueue.main.async {
            ...
            var newRootVC: UITabBarController? = nil
            var rootVC: UINavigationController
            if isShowingChatListVC(), let nav = UiUtils.mainNavVC() {
                rootVC = nav
                ...
                let tabs = UiUtils.makeMainTabVC()
                newRootVC = tabs
                rootVC = tabs.viewControllers!.first as! UINavigationController
            }
            let messageVC =
