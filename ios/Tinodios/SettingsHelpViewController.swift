...
class SettingsHelpViewController: UITableViewController {
    ...
    private func setup() {
        ...
        // Service link under the app name — only when a real site is configured.
        if let privacyUrl = self.privacyUrl, let host = privacyUrl.host {
            var components = URLComponents()
            components.scheme = privacyUrl.scheme
            components.host = host
            serviceLinkLabel.text = components.url?.absoluteString
            serviceLinkLabel.isHidden = false
        } else {
            serviceLinkLabel.text = nil
            serviceLinkLabel.isHidden = true
        }
        // Hide legal rows that have nowhere to go.
        termsOfUse.isHidden = (self.tosUrl == nil)
        privacyPolicy.isHidden = (self.privacyUrl == nil)
        // Server address.
    ...
    @objc func termsOfUseClicked(sender: UITapGestureRecognizer) {
        if let url = self.tosUrl { UIApplication.shared.open(url) }
    }

    @objc func privacyPolicyClicked(sender: UITapGestureRecognizer) {
        if let url = self.privacyUrl { UIApplication.shared.open(url) }
    }
