...
public class RoundImageView: UIImageView {
    ...
    public func set(pub: TheCard?, id: String?, deleted: Bool) {
        if let icon = pub?.photo?.image {
            ...
            // WhatsApp-style: no avatar means a head glyph, never initials —
            // one head for direct chats, two for groups, each on its own tint.
            self.initials = nil
            self.image = RoundImageView.defaultIcon(forType: iconType)
            self.backgroundColor = deleted
                ? Constants.kDeletedBackground
                : RoundImageView.placeholderColors(forType: iconType).background
        }
    ...
    // WhatsApp-style placeholder colors: glyph + circle background per topic type.
    static func placeholderColors(forType iconType: IconType) -> (glyph: UIColor, background: UIColor) {
        switch iconType {
        case .grp:
            // Groups: blue circle, lighter blue two-heads.
            return (UIColor(fromHexCode: 0xff53a6dc), UIColor(fromHexCode: 0xff1d3a57))
        case .slf:
            // Saved messages: green, matching the app accent.
            return (UIColor(fromHexCode: 0xff53d6a0), UIColor(fromHexCode: 0xff17402f))
        default:
            // Direct chats: plum circle, pink single head.
            return (UIColor(fromHexCode: 0xffe8798e), UIColor(fromHexCode: 0xff3b2c33))
        }
    }

    private static func defaultIcon(forType iconType: IconType) -> UIImage {
        let icon: UIImage
        // Filled variants read better than outlines at row size.
        switch iconType {
        case .grp:
        ...
        case .slf:
            icon = UIImage(systemName: "bookmark.fill", withConfiguration: UIImage.SymbolConfiguration(pointSize: 32, weight: .medium, scale: .large))!
        case .p2p:
            icon = UIImage(systemName: "person.fill", withConfiguration: UIImage.SymbolConfiguration(pointSize: 32, weight: .medium, scale: .large))!
        default:
            icon = UIImage(systemName: "person.fill", withConfiguration: UIImage.SymbolConfiguration(pointSize: 32, weight: .medium, scale: .large))!
        }
        return icon.withTintColor(placeholderColors(forType: iconType).glyph).withInset(6)!
    }
