...
class MessageViewController: UIViewController {
    ...
    enum Constants {
        ...
        // Wide enough for '11:26 pm' in caption2 without truncating.
        static let kTimestampWidth: CGFloat = 74
        ...
        // WhatsApp-style palette: OUTGOING messages are green, incoming are neutral.
        // (Upstream Tinode colors this the other way around.)
        // Light: soft green; dark: deep teal-green.
        static let kOutgoingBubbleColorLight = UIColor(fromHexCode: 0xffdcf8c6)
        static let kOutgoingBubbleColorDark = UIColor(fromHexCode: 0xff005c4b)
        // And corresponding text color
        static let kOutgoingTextColorLight = UIColor.darkText
        static let kOutgoingTextColorDark = UIColor(fromHexCode: 0xffe9edef)
        // Incoming: white in light mode, dark slate in dark mode.
        static let kIncomingBubbleColorLight = UIColor.white
        static let kIncomingBubbleColorDark = UIColor(fromHexCode: 0xff202c33)
        // And corresponding font color
        static let kIncomingTextColorLight = UIColor.darkText
        static let kIncomingTextColorDark = UIColor(fromHexCode: 0xffe9edef)
        ...
        // Bold variant for the colored sender name shown above group messages.
        // Kept at caption1 size: bigger fonts overflow kSenderNameLabelHeight and
        // clip ascenders/descenders (the label frame is fixed by the layout).
        static let kSenderNameFontBold = UIFont.systemFont(ofSize: UIFont.preferredFont(forTextStyle: .caption1).pointSize, weight: .semibold)
        static let kTimestampFont = UIFont.preferredFont(forTextStyle: .caption2)
        static let kSenderNameLabelHeight: CGFloat = 18
        // Breathing room between the sender name and the bubble beneath it.
        static let kSenderNameBottomPadding: CGFloat = 5
    ...
    override func traitCollectionDidChange(_ previousTraitCollection: UITraitCollection?) {
        ...
        self.setInterfaceColors()
        // Bubble colors are applied per-cell at fill time from the current
        // trait collection; visible cells keep stale light/dark colors after
        // an appearance switch unless reloaded.
...
extension MessageViewController: UICollectionViewDataSource {
    ...
    private func configureCell(cell: MessageCell, with message: Message, at indexPath: IndexPath) {
        ...
        // WhatsApp-style: don't stamp every bubble. Show the time only on the
        // last message of a run from the same sender, or when more than 5
        // minutes separate this message from the next one.
        if let ts = message.ts, shouldShowTimestamp(for: message, at: indexPath) {
    ...
    // Get sender name
    // WhatsApp's sender-name palette: distinct, readable on the dark bubble.
    private static let kSenderColors: [UIColor] = [
        UIColor(fromHexCode: 0xff53bdeb), UIColor(fromHexCode: 0xffa66bff),
        UIColor(fromHexCode: 0xff25d366), UIColor(fromHexCode: 0xffffa15c),
        UIColor(fromHexCode: 0xffff8fa3), UIColor(fromHexCode: 0xff7ee0c0),
        UIColor(fromHexCode: 0xffe6c86e), UIColor(fromHexCode: 0xff9ab7ff)
    ]

    static func senderColor(for uid: String?) -> UIColor {
        guard let uid = uid, !uid.isEmpty else { return kSenderColors[0] }
        let hash = UInt(uid.hashCode().magnitude)
        return kSenderColors[Int(hash % UInt(kSenderColors.count))]
    }

    // True when this message ends a visual cluster: last in the list, next
    // message is from someone else, or a >5 minute gap follows.
    private func shouldShowTimestamp(for message: Message, at indexPath: IndexPath) -> Bool {
        guard let ts = message.ts else { return false }
        let nextIndex = indexPath.item + 1
        guard nextIndex < messages.count else { return true }
        let next = messages[nextIndex]
        if next.from != message.from { return true }
        guard let nextTs = next.ts else { return true }
        return nextTs.timeIntervalSince(ts) > 5 * 60
    }
    ...
    func senderFullName(for message: Message, at indexPath: IndexPath) -> NSAttributedString? {
        ...
        // WhatsApp-style: bold, colored per sender so group conversations are
        // easy to scan. Color is derived from the sender id so it is stable.
        return NSAttributedString(string: senderName!, attributes: [
            NSAttributedString.Key.font: Constants.kSenderNameFontBold,
            NSAttributedString.Key.foregroundColor: MessageViewController.senderColor(for: message.from)
            ])
    }
...
extension MessageViewController: MessageViewLayoutDelegate {
    ...
    func collectionView(_ collectionView: UICollectionView, fillAttributes attr: MessageViewLayoutAttributes) {
        ...
        if isAvatarVisible {
            attr.avatarFrame = CGRect(x: 0, y: cellSize.height - Constants.kAvatarSize, width: Constants.kAvatarSize, height: Constants.kAvatarSize)
        } else {
            attr.avatarFrame = .zero
        }
        // senderNameFrame is set below, once the container frame is known:
        // the name renders inside the bubble.
        ...
        let senderNameBlockHeight = senderNameLabelHeight > 0 ? senderNameLabelHeight + Constants.kSenderNameBottomPadding : 0
        attr.contentFrame = CGRect(x: contentInset.left, y: contentInset.top + senderNameBlockHeight, width: attr.containerFrame.width - contentInset.left - contentInset.right, height: attr.containerFrame.height - contentInset.top - contentInset.bottom - senderNameBlockHeight - (showUploadProgress ? Constants.kProgressViewHeight : 0))

        if senderNameLabelHeight > 0 {
            // Name inside the bubble, top-left, in cell coordinates
            // (senderNameLabel is a sibling of the container, not a child).
            // Container-local coordinates: the label is a subview of containerView.
            attr.senderNameFrame = CGRect(
                x: contentInset.left,
                y: contentInset.top,
                width: attr.containerFrame.width - contentInset.left - contentInset.right,
                height: senderNameLabelHeight)
        }
    ...
    func calcCellSize(forItemAt indexPath: IndexPath) -> CGSize {
        ...
        let nameVisible = !message.isDeleted && shouldShowAvatar(for: message, at: indexPath)
        let containerHeight = calcContainerSize(for: message, avatarsVisible: hasAvatars, progressVisible: showProgress, senderNameVisible: nameVisible).height
    ...
    func calcCellHeightFromContent(
        ...
        // The sender name is inside the container now; containerHeight covers it.
        let totalLabelHeight: CGFloat = newDateLabelHeight + containerHeight + (progressVisible ? Constants.kProgressViewHeight : 0)
    ...
    func calcContainerSize(for message: Message, avatarsVisible: Bool, progressVisible: Bool, senderNameVisible: Bool = false) -> CGSize {
        ...
        if senderNameVisible {
            // WhatsApp-style: the sender name lives INSIDE the bubble, above the text.
            size.height += Constants.kSenderNameLabelHeight + Constants.kSenderNameBottomPadding
        }
