...
class LoginViewController: UIViewController {
    ...
    // input fields, welcome heading.
    private func styleWhatsAppLook() {
        let green = UIColor(fromHexCode: 0xff00a884)

        // Classic API: the deployment target (iOS 14) predates UIButton.Configuration.
        signInButton.backgroundColor = green
        signInButton.setTitleColor(.white, for: .normal)
        signInButton.setTitleColor(UIColor.white.withAlphaComponent(0.4), for: .disabled)
        signInButton.titleLabel?.font = UIFont.systemFont(ofSize: 17, weight: .semibold)
        signInButton.contentEdgeInsets = UIEdgeInsets(top: 12, left: 40, bottom: 12, right: 40)
        signInButton.layer.cornerRadius = 22
        signInButton.layer.masksToBounds = true

        for field in [userNameTextEdit, passwordTextEdit] {
            field?.borderStyle = .none
            field?.backgroundColor = .secondarySystemBackground
            field?.layer.cornerRadius = 12
            field?.layer.masksToBounds = true
            // Comfortable inner padding, since .none removes the default inset.
            let pad = UIView(frame: CGRect(x: 0, y: 0, width: 14, height: 1))
            field?.leftView = pad
            field?.leftViewMode = .always
        }

        serviceNameLabel.text = NSLocalizedString("Welcome to BLML", comment: "Login screen heading")
        serviceNameLabel.font = UIFont.systemFont(ofSize: 24, weight: .bold)
        serviceNameLabel.textColor = .label
        serviceNameLabel.adjustsFontSizeToFitWidth = true
        serviceNameLabel.minimumScaleFactor = 0.5
    }
