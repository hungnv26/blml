...
const BASE_APP_NAME = 'BLML';
const APP_NAME = BASE_APP_NAME + '/' + (_version_js__WEBPACK_IMPORTED_MODULE_0__.PACKAGE_VERSION || '0.25');
const API_KEY = 'AQAAAAABAAC-d-KsShjNeHzNi7myV36_';
const KNOWN_HOSTS = {
  hosted: 'chat.blml.app',
...
function imageCrop(mime, objURL, left, top, width, height, scale, naturalScale) {
  return new Promise((resolve, reject) => {
    const img = new Image();
    img.crossOrigin = 'Anonymous';
    img.onerror = _ => {
    ...
    img.onload = _ => {
      URL.revokeObjectURL(img.src);
      const ns = naturalScale > 0 ? naturalScale : 1;
      const srcLeft = left * ns,
        srcTop = top * ns;
      const srcWidth = width * ns,
        srcHeight = height * ns;
      const outWidth = Math.max(1, Math.round(Math.min(srcWidth, _config_js__WEBPACK_IMPORTED_MODULE_1__.AVATAR_SIZE)));
      const outHeight = Math.max(1, Math.round(Math.min(srcHeight, _config_js__WEBPACK_IMPORTED_MODULE_1__.AVATAR_SIZE)));
      let canvas = document.createElement('canvas');
      ...
      ctx.drawImage(img, srcLeft, srcTop, srcWidth, srcHeight, 0, 0, outWidth, outHeight);
...
function updateFavicon(count) {
  ...
  document.title = (count > 0 ? '(' + count + ') ' : '') + 'BLML';
}
...
class AvatarCrop extends (react__WEBPACK_IMPORTED_MODULE_0___default().PureComponent) {
  ...
  handleChange(left, top, width, height, scale, naturalScale) {
    this.setState({
      left: left,
      top: top,
      width: width,
      height: height,
      scale: scale,
      naturalScale: naturalScale || 1
    });
  }
  handleSubmit() {
    (0,_lib_blob_helpers_js__WEBPACK_IMPORTED_MODULE_3__.imageCrop)(this.props.mime, this.props.avatar, this.state.left, this.state.top, this.state.width, this.state.height, this.state.scale, this.state.naturalScale).then(img => {
...
class Cropper extends (react__WEBPACK_IMPORTED_MODULE_0___default().Component) {
  ...
  positionAll(panX, panY, zoom) {
    ...
    this.props.onChange((left + this.cutoutRect.left - this.bBoxRect.left) / zoom, (top + this.cutoutRect.top - this.bBoxRect.top) / zoom, this.cutoutRect.width / zoom, this.cutoutRect.height / zoom, zoom, this.naturalScale || 1);
  }
  ...
  initScaling() {
    ...
    const natural = this.preview.current.naturalWidth || imgRect.width;
    this.naturalScale = imgRect.width > 0 ? natural / imgRect.width : 1;
