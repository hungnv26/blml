...
// returns a promise.
// naturalScale: source pixels per display pixel of the cropper preview. The crop
// rectangle arrives in display coordinates; without this the output resolution
// was capped at the on-screen cropper size, which is why avatars looked soft.
export function imageCrop(mime, objURL, left, top, width, height, scale, naturalScale) {
  return new Promise((resolve, reject) => {
    const img = new Image();
    img.crossOrigin = 'Anonymous';
    img.onerror = _ => {
    ...
    img.onload = _ => {
      ...
      // drawImage's source rectangle is in the image's natural pixels, so convert
      // the display-space crop rect before sampling.
      const ns = naturalScale > 0 ? naturalScale : 1;
      const srcLeft = left * ns, srcTop = top * ns;
      const srcWidth = width * ns, srcHeight = height * ns;

      // Render at the source resolution actually available, capped so a huge
      // upload doesn't produce a needlessly large avatar. Never upscale.
      const outWidth = Math.max(1, Math.round(Math.min(srcWidth, AVATAR_SIZE)));
      const outHeight = Math.max(1, Math.round(Math.min(srcHeight, AVATAR_SIZE)));
      ...
      canvas.toBlob((blob) => {
        ...
        canvas = null;
        if (blob) {
          resolve({mime: mime, blob: blob, width: outWidth, height: outHeight});
        } else {
          reject(new Error("Unsupported image format"));
        }
      }, mime);
